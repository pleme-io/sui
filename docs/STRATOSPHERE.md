...
- **S1 language corpus 25 → 116 passing real Nix tests** (21 honest known_broken gaps; vendored from CppNix's own
...
|---|---|---|---|---|
| **S1 Language** | Corpus **116/137 ≈ 64%** eval-okay fixtures (`lang_corpus.rs`, 116 active + 21 `known_broken/`). Builtins **~89/120 ≈ 74% IMPLEMENTED**; per-builtin PARITY far thinner — 19 module-level smoke probes only, **~100 builtins have zero dedicated coverage**. Grammar: all 25 productions dispatched, **none enumerated**. | Builtin registry keys (must build `builtin_names()`). **Grammar has NO owned enum** — rnix `ast` is foreign rowan CST; `sui_ir::ir::{BinOp,UnaryOp}` exist but are "wired into nothing". | `(defbuiltincase)` (M0) + `(deflangcase)` for productions/operators. Builtin bijection is a strong registry gate; grammar is a WEAK name-scan only (no compile-tier claim). | Byte-parity soundness is C2 (fixture `.exp` correctness untrusted). Per-production compile-tier enumeration is INFEASIBLE (foreign AST). |
| **S2 Closure** | **Walk LANDED 2026-08-18** — `bisect_drv_all` is visit-all (pair seen-set + path-keyed `parsed` memo, multimap name pairing, depth cap deleted). Enumeration rows still unbuilt. `build-parity.yml` = 2-row NAR scaffold. Walk substrate ~80% ready (`BuildClosure::compute`, `closure.rs:32`). | `BuildClosure::compute` node set over both toplevels. | `(defclosure-subject)` + machine-walked `NodeCoverage` rows + `Divergence::MissingNode`. One code change: replace `bisect_drv`'s first-child descent with a visit-all walk reusing the `parsed` memo. | **Corrected 2026-08-18: `blocker=MemoryWall` was misattributed to the WALK.** The 20,827-node / ~2.94 MB-per-node figure is the cost of the EVAL that instantiates the toplevel, not of walking it. Measured: the toplevel closure is 3,056 `.drv` nodes / 14.96 MB of ATerm; the walk's own retention at cid scale is **~580 MB** with both sides' parsed derivations held — ~600x cheaper per node, and it does not hit the wall at any scale on this box. The eval `parity-bisect` shells out to still costs 10 GB, so end-to-end on a toplevel is still blocked — but by the eval, and the distinction decides what to fix. |

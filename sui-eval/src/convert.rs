...
///
/// Note: thunks and closures cannot cross the boundary — they are converted
/// to `VMValue::Null` / `VMValue::Null` respectively, since the VM and
/// tree-walker have incompatible closure/thunk representations.
#[must_use]
pub fn eval_to_vm(val: &Value, interner: &mut Interner) -> VMValue {
    match val {
        ...
        // Closures, builtins, and thunks cannot cross the boundary.
        Value::Lambda(_) | Value::Builtin(_) | Value::Thunk(_) => VMValue::Null,
    }
}

...
> 3. **Diagnosis re-confirmed by parallel forensics (85% confidence): the dominant term is INHERENT, not reducible.** sui's attr maps are `im_rc` HAMTs (O(1) structural-sharing clones), keys are interned `Symbol(u32)`, payloads are `Rc`-shared — so the suspected "flat-cache × 50 layers" multiplier is FALSE, and the ~15–23 GB is the live derivation-closure of the Merkle hash. **Only streaming bottom-up finalize-then-release (§2) bends the curve** — eviction/GC is measured-counterproductive on inherent retention (the seal's REDUCIBLE-vs-INHERENT Gate 0, now canonical in `theory/ALGORITHMIC-PROWESS-SEAL.md §V.2`). Streaming-release is the confirmed load-bearing next fix, gated by the `SUI_NO_STREAM_RELEASE=1` byte-parity differential (C-class ceiling: memory is an external-world resource; the invariant is CI-forcing-function-gated, never a truly-unrep type).
